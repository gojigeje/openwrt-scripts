#!/bin/bash
# ----------------------------------------------------------------------------------
# @name    : control.mjpgstreamer_nosave.sh
# @version : 0.1
# @date    : 2014/05/28 22:26 WIB
#
# ABOUT
# ----------------------------------------------------------------------------------
# Script to control mjpg_streamer // no save, only streaming 
#
# AUTHOR
# ----------------------------------------------------------------------------------
...
cek_running() {
  stat_mjpg=$(ps | grep -v grep | grep -v control.mjpgstreamer_nosave.sh | grep -c mjpg_streamer)
  if [ "$stat_mjpg" -gt "0" ]
     then
        smjpg="ON"
     else
